...
Two atomic windows:
  ...
  2. Consume/confirm each line, create Order, finalize reservation status
"""
import uuid
...
from domain.enums import HoldStatus, OrderStatus, OutboxTaskType, ReservationStatus
from infra.db.models import OrderItemModel, OrderModel, ProviderModel
...
class ConfirmReservationUseCase:
    ...
    async def execute(self, reservation_id: uuid.UUID) -> Reservation:
        ...
        async with atomic(self._session):
            ...
            for item in reservation.items:
                ...
                if provider and provider.type.value == "internal":
                        ...
                        try:
                            ...
                            if result.success:
                                ...
                                await self._outbox_repo.enqueue(
                                    ...
                                    payload={"reservation_id": str(reservation_id), "item_id": str(item.id), "provider_id": str(item.provider_id), "provider_ref": item.provider_ref},
                                    idempotency_key=key,
                                )
                        except TimeoutError:
                            has_pending_fulfilment = True
            ...
            if has_needs_resolution:
                await self._compensate_confirmed(reservation, confirmed_items)
    ...
    async def _compensate_confirmed(
        self, reservation: Reservation, confirmed_item_ids: list[uuid.UUID]
    ) -> None:
        for item in reservation.items:
    ...
    def _build_adapter(self, provider: ProviderModel):
